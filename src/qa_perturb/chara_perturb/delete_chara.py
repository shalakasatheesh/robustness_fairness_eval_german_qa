...
class DeleteChara():
    ...
    def execute_deletion(self,  word: str):
        '''
        Function to alter the word by deleting a character at a 
        ...
        for _ in range(self.max_perturbs): 
            ...
            if len(indices) > self.length_of_word_to_perturb: 
                ...
                # print("Deleting", new_word[self.chosen_index], "at index", self.chosen_index, "from", new_word)
                # print("Result:", new_word[:self.chosen_index]+new_word[self.chosen_index+1:])
                new_word = new_word[:self.chosen_index]+new_word[self.chosen_index+1:]  
    ...
    def delete_chara(self):
        ...
        for index, sample in enumerate(tqdm(self.data)):
            ...
            if self.pos_tag is None: # get random words to perturb
                ...
                self.name = 'delete_chara_random_word'
                ...
                self.name = 'delete_chara_random_verb'
            ...
            for token in random_words:
                ...
                try:
                    new_word = self.execute_deletion(word)
