...
class ReplaceChara():
    ...
    def execute_replacement(self, word: str):
            
        '''
        Function to alter the word by replacing a character at a 
        randomly chosen index with a randomly chosen ASCII character 
        ...
        for _ in range(self.max_perturbs): 
            self.chosen_index = None
            chosen_chara = random.choice(string.ascii_letters).lower()
            ...
            if len(indices) > self.length_of_word_to_perturb: 
                ...
                # print("Replacing", new_word[self.chosen_index], "at index", self.chosen_index, "from", new_word)
                # print("Result:", new_word[:self.chosen_index]+chosen_chara+new_word[self.chosen_index+1:])
                new_word = new_word[:self.chosen_index]+chosen_chara+new_word[self.chosen_index+1:]  
    ...
    def replace_chara(self):
        ...
        for index, sample in enumerate(tqdm(self.data)):
            ...
            if self.pos_tag is None: # get random words to perturb
                ...
                self.name = 'replace_chara_random_word'
                ...
                self.name = 'replace_chara_random_verb'
            ...
            for token in random_words:
                ...
                try:
                    new_word = self.execute_replacement(word)
