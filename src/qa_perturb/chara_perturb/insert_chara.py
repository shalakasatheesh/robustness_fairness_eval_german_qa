...
class InsertChara():
    ...
    def execute_insertion(self, word: str):
        '''
        Function to alter the word by inserting a randomly chosen ASCII 
        character at a randomly chosen index between the first and last letters
        ...
        for _ in range(self.max_perturbs): 
            ...
            if len(indices) > self.length_of_word_to_perturb: 
                ...
                # print("Inserting", chosen_chara, "at index", self.chosen_index, "from", new_word)
                # print("Result:", new_word[:self.chosen_index]+chosen_chara+new_word[self.chosen_index:])
                new_word = new_word[:self.chosen_index]+chosen_chara+new_word[self.chosen_index:]   
    ...
    def insert_chara(self):
        ...
        for index, sample in enumerate(tqdm(self.data)):
            ...
            if self.pos_tag is None: # get random words to perturb
                ...
                self.name = 'insert_chara_random_word'
                ...
                self.name = 'insert_chara_random_verb'
            ...
            for token in random_words:
                ...
                try:
                    new_word = self.execute_insertion(word)
