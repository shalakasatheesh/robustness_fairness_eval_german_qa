...
class SwapChara():
    ...
    def execute_swap(self, word: str):
            
        '''
        Function to alter the word by swapping a character at a 
        random index with another in the same word
        ...
        for _ in range(self.max_perturbs): 
            ...
            if len(indices) > self.length_of_word_to_perturb: 
                ...
                # print("Swapping", new_word[self.chosen_index], "at index", self.chosen_index, "from", new_word)
                # print("Result:", new_word[:self.chosen_index]+new_word[self.chosen_index+1]+new_word[self.chosen_index]+new_word[self.chosen_index+2:])
                new_word = new_word[:self.chosen_index]+new_word[self.chosen_index+1]+new_word[self.chosen_index]+new_word[self.chosen_index+2:] 
    ...
    def swap_chara(self):
        ...
        for index, sample in enumerate(tqdm(self.data)):
            ...
            if self.pos_tag is None: # get random words to perturb
                ...
                self.name = 'swap_chara_random_word'
            ...
            for token in random_words:
                ...
                try:
                    new_word = self.execute_swap(word)
